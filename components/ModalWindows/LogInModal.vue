<template>
  <v-dialog max-width="400" v-model="show">
    <div class="modal-window">
      <img
        src="~/assets/images/modal-window-log.png"
        class="modal-window_logo"
      />
      <div @click="fbLogin()" class="modal-window_facebook">
        <img
          src="~/assets/images/banner-facebook.png"
          class="modal-window_facebook_img"
        />
      ...
      <div class="c-input modal-window_input">
        <input v-model="password" type="password" class="c-input__input " :class="{'c-input__input__error': 'password' in error && error.password }" placeholder="Пароль" />
      ...
      <div class="modal-window_under-footer">
        ...
        <button @click="show = !show, $modals.showModal({modal: 'SelectRole'})"" class="modal-window_under-footer_regbutton">
          Зарегистрироваться
        </button>
      </div>
    </div>
