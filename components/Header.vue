<template>
  <div class="header">
        ...
        <div v-if="user.role === 1" class="c-user">
          ...
          <div class="c-user__profile-block">
            ...
            <div @click.prevent="showMenu()" @blur="closeMenu()" class="c-user__profile">
              ...
              <h2 class="c-user__username">{{ user.name }}</h2>
            </div>
        ...
        <div v-if="user.role === 2" class="c-user">
          ...
          <div class="c-user__profile-block">
            ...
            <transition name="fade">
              <div v-show="dropdownMenu" class="c-user__drop-down l-drop-down">
                <div class="l-drop-down__list-items">
                  <div @click="$router.push(`/specialist-projects-manage?id=${user.user_id}&type=Overview&status=created`), dropdownMenu = false" class="l-drop-down__list-item l-drop-down__padding">
                    <img class="l-drop-down__icon" src="~/assets/images/icons/profile-icon.svg" alt="">
                    <h2 class="l-drop-down__title">Обзор</h2>
                  </div>

                  <div @click="$router.push(`/specialist-projects-manage?id=${user.user_id}&type=Projects`), dropdownMenu = false" class="l-drop-down__list-item l-drop-down__padding">
                    <img class="l-drop-down__icon" src="~/assets/images/icons/projects-icon.svg" alt="">
                    <h2 class="l-drop-down__title">Проекты</h2>
                  </div>

                  <div @click="$router.push(`/specialist-projects-manage?id=${user.user_id}&type=Reviews`), dropdownMenu = false" class="l-drop-down__list-item l-drop-down__padding">
                    <img class="l-drop-down__icon" src="~/assets/images/icons/reviews-icon.svg" alt="">
                    <h2 class="l-drop-down__title">Отзывы</h2>
                  </div>

                  <div @click="$router.push(`/specialist-projects-manage?id=${user.user_id}&type=Requests`)" class="l-drop-down__list-item l-drop-down__padding">
                    <img class="l-drop-down__icon" src="~/assets/images/icons/mail-icon.svg" alt="">
    ...
    <div class="header-bottom">
      <a @click="$router.push('/projects-client-special')" class="header-bottom_btn">Проекты</a>
      <div style="width: 1px; height: 20px; background: #E5E5E5;"></div>
      <a class="header-bottom_btn">Специалисты</a>
      <div style="width: 1px; height: 20px; background: #E5E5E5;"></div>
      <a class="header-bottom_btn">Блог</a>
    </div>
    <!-- <GlobalModal /> -->
  </div>
</template>
